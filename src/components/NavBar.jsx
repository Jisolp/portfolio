    ...
    <nav className="bg-transparent w-full flex py-6 justify-between items-center navbar">
      <ul className="bg-transparent list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav,index) => (
          <li key={nav.id} className={`font-poppins font-normal cursor-pointer text-[17px] ${index === navLinks.length-1 ? 'mr-0' : 'mr-10'} text-black mr-10 font-semibold hover:text-yellow-100`}>
            <a href={`#${nav.id}`}>
              {nav.title}
            </a>
          </li>
